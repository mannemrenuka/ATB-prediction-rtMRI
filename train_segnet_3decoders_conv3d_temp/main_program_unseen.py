...
path = 'segnet_conv3d_temp_unseen_3decoders'
...
#fixed_dim =68
frames = 24
segnet_model=model.Segnet(n_ops ,height, width,frames)
...
model_dir = '/home/user/home2/data/navaneetha/models/segnet_temp__unseen_3decoders_binary/'
...
#mat_dir = '/home2/data/navaneetha/data/'
...
for i in range(0,len(subs)):
	...
	test_matrix = subs[i]
	print(test_matrix)
	X_train,Y_train,X_test,Y_test,name,name_test=image_utils.imageSegmentationGenerator(in_dir,out_dir,n_ops,height,width,train_matrix,test_matrix,frames)

	print ('X_train.shape',X_train.shape)
	print ('Y_train[0].shape',Y_train[0].shape)
	print (Y_train[1].shape)
	print (Y_train[2].shape)
	#print (type(X_trainf[0]))
	print (X_test.shape)
	...
	#segnet_model=model.Segnet(n_ops ,height, width,frames, fixed_dim =68)
	#segnet_model.summary()										
	...
	#history=segnet_model.fit(X_train, (Y_train), epochs=epoch,batch_size=1,validation_split = 0.1,verbose=1,callbacks=callbacks)
	#preds = image_utils.predicted_output(segnet_model,X_test,height,width,n_ops)
